...
/datum/nano_module/program/reports/proc/load_report(mob/user)
	...
	if(choice in choices)
		...
		if(editing == "View")
			if(!(chosen_report.get_file_perms(get_access(user), user) & NTOS_READ_ACCESS))
				to_chat(user, "<span class='warning'>You lack access to view this report.</span>")
				return
			can_view_only = 1
		else
			if(!(chosen_report.get_file_perms(get_access(user), user) & NTOS_WRITE_ACCESS))
...
/datum/nano_module/program/reports/Topic(href, href_list)
	...
	if(href_list["save"])
		if(!selected_report)
			return 1
		if(!(selected_report.get_file_perms(get_access(user), user) & NTOS_READ_ACCESS))
			return 1
	...
	if(href_list["edit"])
		...
		if(!field || !(field.get_perms(get_access(usr), usr) & NTOS_WRITE_ACCESS))
			return 1
		field.ask_value(user) //Handles the remaining IO.
		return 1
	if(href_list["print"])
		if(!selected_report || !(selected_report.get_file_perms(get_access(user), user) & NTOS_READ_ACCESS))
			return 1
