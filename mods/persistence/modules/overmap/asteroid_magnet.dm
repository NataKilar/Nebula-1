...
/obj/machinery/asteroid_magnet
	...
	desc = "A massive solenoid used to attract asteroids from nearby fields for mineral acquisition."
...
/obj/machinery/asteroid_magnet/proc/generate_asteroid(var/obj/effect/overmap/event/meteor/asteroid)
	...
	var/turf/rock_turf = class.rock_type
	var/turf/mineral_turf = class.mineral_type
	...
	for(var/turf/T in target_turfs)
		...
		var/det = Clamp((rand(ASTEROID_SIZE, 3*ASTEROID_SIZE) - dist), 0, 3*ASTEROID_SIZE)
		var/rock_lb = 2
		var/rock_ub = 2*ASTEROID_SIZE

		var/min_lb = 2*ASTEROID_SIZE + 1
		var/min_ub = 3*ASTEROID_SIZE

		// This doesn't play nicely with a switch statement.
		if(det >= rock_lb && det <= rock_ub)
			T.ChangeTurf(rock_turf)
		else if(det >= min_lb && det <= min_ub)
			T.ChangeTurf(mineral_turf)
...
/obj/machinery/asteroid_magnet/proc/get_asteroid()
	var/obj/effect/overmap/curr_sector = map_sectors["[z]"]
	if(!curr_sector)
		return "Could not find orientation in space!"
	if(istype(curr_sector, /obj/effect/overmap/visitable/ship/landable) || istype(curr_sector, /obj/effect/overmap/visitable/sector/exoplanet))
